...
void aie_multiplier_stream(input_stream<TYPE>*, input_stream<TYPE>*, output_stream<TYPE>*);
void aie_multiplier_window(input_window<TYPE>*, input_window<TYPE>*, output_window<TYPE>*);
void aie_multiplier_free_running_scalar(input_stream<TYPE>*, input_stream<TYPE>*, output_stream<TYPE>*);
//void aie_multiplier_circular_buffer_vector(input_circular_buffer<TYPE, adf::extents<BUFFER_SIZE>>&, input_circular_buffer<TYPE, adf::extents<BUFFER_SIZE>>&, output_circular_buffer<TYPE, adf::extents<BUFFER_SIZE>>&) {
//void aie_multiplier_buffer_scalar(input_buffer<TYPE>& __restrict in0, input_buffer<TYPE>& __restrict in1, output_buffer<TYPE>& __restrict out);
...
class simpleGraph : public graph {
   ...
   public:
    ...
    simpleGraph() {
...
#if defined (WINDOW)
    	multiplier = kernel::create(aie_multiplier_window);
#elif defined (STREAM)
    	multiplier = kernel::create(aie_multiplier_stream);
#elif defined (FREE)
    	multiplier = kernel::create(aie_multiplier_free_running);
#elif defined (BUFFER_SCALAR) 
    	multiplier = kernel::create(aie_multiplier_buffer_scalar);
#elif defined (BUFFER_VECTORIZED)
    	multiplier = kernel::create(aie_multiplier_buffer_vector);
#endif
        source(multiplier) = "kernel.cc";
        ...
        //pl_in0 = input_plio::create("DataIn0");
        //pl_in1 = input_plio::create("DataIn1");
        //pl_out = output_plio::create("DataOut");

	connect METHOD net0(pl_in0.out[0], multiplier.in[0]);
        connect METHOD net1(pl_in1.out[0], multiplier.in[1]);
        connect METHOD net2(multiplier.out[0], pl_out.in[0]);


        runtime<ratio>(multiplier) = 0.1;
    };
};
